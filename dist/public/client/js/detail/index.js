const data = {
     colorId: "6789fa4529c76eab42b24c44",
     sizeId: "67862d1224e1760b15b700fa",
};
               ...
               axios.put(`${location.href}/getSize`, {
                    ...
                    .then(res => {
                         if (res.status == 200 && res.data.listSizes && res.data.listSizes.length > 0) {
                              // closeLoader()
                              const sizeP = document.querySelector('sl-radio-group[size-product]')
...
listSize.forEach(size => {
     size.addEventListener('click', () => {
          const colorChecked = document.querySelector('[color-product] input:checked')
          if (colorChecked) {
               const colorId = colorChecked.getAttribute('color-id')
               axios.post(`${location.href}/getItem`, {
                         color: colorId,
                         size: size.value
                    })
                    .then(res => {
...
const addCart = () => {
     ...
     btnAdd.addEventListener('click', () => {
          ...
          if (!link) return
          const colorElement = document.querySelector('[color-product] input:checked')
          if (!colorElement) {
               localStorage.setItem(
                    "alert-error",
                    JSON.stringify({
